...
if (!$con){
    ...
    if (isset($_POST['registerbtn'])){
        ...
        if ($email == ""){
            header("Location: signupintern.php?error=Enter your email.");
            exit();
        }
        //checking if name field is empty
        else if ($name == ""){
            header("Location: signupintern.php?error=Enter your name.");
            exit();
        }
        //verification of email format
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            header("Location: signupintern.php?error=Invalid Email. Please check carefully.");
            exit();
        }
        //checking for duplicate emails
        else if ($FilteringResult['Email']){
            header("Location: signupintern.php?error=Email has already been taken by another user.");
            exit();
        }
        //filtering of characters in name (must not have numbers/special characters
            ...
            header("Location: signupintern.php?error=Name must not contain numbers and special characters.");
            exit();
            }
        ...
        if ($password1 != $password2){
                header("Location: signupintern.php?error=Passwords does not match.");
                exit();
            }
            //checking if the 2 passwords are 8 characters long or above
            else if (strlen($password1) < 8 && strlen($password2) < 8 ){
                header("Location: signupintern.php?error=Passwords must be at least 8 characters long.");
                exit();
            }
            //checking if the 2 passwords have numbers/special characters (must have)
                ...
                header("Location: signupintern.php?error=Passwords must contain numbers and special characters.");
                exit();
            }
            ...
            header("Location: signupintern.php?error=Input your password.");
            exit();
        }
    }
        ...
        <form method="POST" style="margin-top: 30px;">
            <h4>SIGN UP</h4>
                ...
                <?php if (isset($_GET['error'])) { ?>
                    <div class="alert alert-danger d-flex align-items-center d-flex justify-content-center" style="width:300px; margin-top: 5px;height: 44px; font-size: 14px; padding: 0px;"><?php echo $_GET['error']; ?> </div>
                <?php } ?>
                    ...
                    <p style="margin-bottom:0px;margin-top: 15px;font-size: 13px;">Your Company:</p>
