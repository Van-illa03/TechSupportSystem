...
if (isset($_POST['login'])){
    ...
    $FilteringResult= mysqli_fetch_assoc($Filtering);
     ...
     if ($email == ""){
        header("Location: supportlogin.php?error=Enter your email.");
        exit();
    }
    //verification of email format
    else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: supportlogin.php?error=Invalid Email. Please check carefully.");
        exit();
    }
    else if ($FilteringResult == null) {
        header("Location: supportlogin.php?error=Your account does not exist.");
        exit();
    }
    //checking if passwords fields are not empty
        ...
        if ($FilteringResult['Password'] != $password){
            header("Location: supportlogin.php?error=Incorrect password.");
            exit();
        }
        else if ($supportcode != $getcodes['SupportCode']){
            header("Location: supportlogin.php?error=Incorrect Support code.");
            exit();
        }
        ...
        header("Location: supportlogin.php?error=Input your password.");
        exit();
    }
}
?>
