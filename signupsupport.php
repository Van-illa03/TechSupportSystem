...
if (!$con){
    ...
    if (isset($_POST['registerbtn'])){
        ...
        $supportcode = $_POST['susc']; //sign up support code
        ...
        if ($name == ""){
            header("Location: signupsupport.php?error=Enter your name.");
            exit();
        }
        //checking if name field is empty
        else if ($email == ""){
            header("Location: signupsupport.php?error=Enter your email.");
            exit();
        }
        //verification of email format
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            header("Location: signupsupport.php?error=Invalid Email. Please check carefully.");
            exit();
        }
        //checking for duplicate emails
        else if ($FilteringResult['Email']){
            header("Location: signupsupport.php?error=Email has already been taken by another user.");
            exit();
        }
        //filtering of characters in name (must not have numbers/special characters
            ...
            header("Location: signupsupport.php?error=Name must not contain numbers and special characters.");
            exit();
            }
        //checking if passwords fields are not empty
            ...
            if ($password1 != $password2){
                    header("Location: signupsupport.php?error=Passwords does not match.");
                    exit();
                }
                //checking if the 2 passwords are 8 characters long or above
                else if (strlen($password1) < 8 && strlen($password2) < 8 ){
                    header("Location: signupsupport.php?error=Passwords must be at least 8 characters long.");
                    exit();
                }
                //checking if the 2 passwords have numbers/special characters (must have)
                    ...
                    header("Location: signupsupport.php?error=Passwords must contain numbers and special characters.");
                    exit();
                }
                else if ($supportcode != $getcodes['SupportCode']){
                    header("Location: signupsupport.php?error=Incorrect support code.");
                    exit();
                }
                ...
                window.location.replace("supportlogin.php");
                </script>';
                }
        }
        else {
            header("Location: signupsupport.php?error=Input your password.");
            exit();
        }
    }
    ...
    <div class="d-flex">
        ...
        <form method="POST" style="margin-top: 15px;">
            ...
            <div class="d-flex flex-row justify-content-center">
                <input type="password" class="form-control" id="susc" placeholder="Support Code" name="susc" style="width:130px; margin:10px;">
            </div>
            ...
            <a href="supportlogin.php"><button class="btn btn-info">Account Login</button></a>
        </div>
            </div>
